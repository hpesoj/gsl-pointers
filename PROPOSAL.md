...
Given that `indirect` in many ways behaves like a pointer, and `T&` does not implicitly convert to `T*`, this behaviour may raise a few eyebrows. Here we explain the reasoning behind this decision.
...
If users can get used to "pointer-like" indirection operators on a value-like type, then they can get used to "reference-like" construction from `T&` for a pointer-like type.

A second objection may be that "rebinding" on reassignment is surprising behaviour; if `indirect<T>` is initiailized like `T&`, users might expect assignment from `T&` to assign to the referenced object as with assignment to `T&`. However, rebinding on assignment is already the behaviour exhibited by `reference_wrapper`:
